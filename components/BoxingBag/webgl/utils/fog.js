...
// Permutations
i = mod289(i);
vec4 p = ppermute( ppermute( ppermute(
...
//Normalise gradients
vec4 norm = ttaylorInvSqrt(vec4(dot(p0,p0), dot(p1,p1), dot(p2, p2), dot(p3,p3)));
p0 *= norm.x;
p1 *= norm.y;
p2 *= norm.z;
p3 *= norm.w;
...
function roundedFog() {
  ...
  #ifdef USE_FOG
    float distanceFactor = smoothstep(1.5, 2.0, distance(position.xz, vec2(.0)));
    float cameraFactor = smoothstep(0.3, 0.8, distance(position.xz, cameraPosition.xz));
    vFogDepth = distanceFactor;
  #endif
  `
...
function noiseFog() {
  ...
  ShaderChunk.fog_fragment = `
      ...
      vec3 noiseSampleCoord = (vWorldPosition * 0.25);
      ...
      fogFactor += smoothstep(1.5, 2.0, distance(vWorldPosition, vec3(0.0)));
...
class FogMaterial extends MeshStandardMaterial {
  constructor(props) {
    super(props);
    ...
    this.uniforms = {
      uFogTime: {value: 0},
      uFogSpeed: {value: 2.4},
      uFogDistortion: {value: 1.3},
      uFogDirection: {value: new Vector3(1, 0, 0)},
      uFogScale: {value: new Vector3(2, 1, 2)},
      uFogPosition: {value: new Vector3(0, -1.01, 0)}, //
    }
  }

  update(time) {
    this.uniforms.uFogTime.value += 0.0005 * time.delta
  }
